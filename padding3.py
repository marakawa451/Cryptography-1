...
pad_range = ["00", "01", "02", "03", "04", "05", "06", "07", "08", "09"]
...
def search(ct, pad, rquery):
  for i in allhex:
        xori = xorb(bytes.fromhex(i),bytes.fromhex(ct))
        xori2 = xorb(xori, bytes.fromhex(pad))
        strxori = str(xori2.hex())
        r = requests.get('http://crypto-class.appspot.com/po?er=' + rquery + strxori + endstring)
...
def main():
    for i in range(9):
       ct_byte = splice_ct()
       qstring = new_qstring()
       pt = search(ct_byte, "0" + str(i) , qstring)
       print(pt)
       new_pad = triple(ct_byte, pt, "0" + str(i + 1))
       endstring = new_endstring(new_pad)
       print(new_pad)
       print(qstring)
       print(endstring)
